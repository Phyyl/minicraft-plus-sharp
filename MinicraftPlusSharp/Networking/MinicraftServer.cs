...
{
    public class MinicraftServer : JavaThread, MinicraftProtocol
    {
        ...
        private static readonly int UPDATE_INTERVAL = 10; // measured in seconds
            ...
            : base("MinicraftServer")
        {
            ...
            try
            {
                Console.WriteLine("Opening server socket...");
                socket = new TcpListener(IPAddress.Any, port);
                Start();
            }
            catch (IOException ex)
            {
                Console.Error.WriteLine("Failed to open server socket on port " + port);
                ex.PrintStackTrace();
            }
        }
        ...
        public void run()
        {
            if (Game.debug) Console.WriteLine("Server started.");

            Timer gameUpdateTimer = new Timer(5000);

            gameUpdateTimer.Elapsed += delegate
            {
                gameUpdateTimer.Interval = UPDATE_INTERVAL * 1000;
                UpdateGameVars();
            };
            ...
            try
            {
                while (socket != null)
                {
                    MinicraftServerThread mst = new MinicraftServerThread(socket.AcceptTcpClient(), this);
                    if (mst.isConnected())
                        threadList.add(mst);
                }
            }
            catch (SocketException ex)
            { // this should occur when closing the thread.
            }
            catch (IOException ex)
            {
    ...
    public MinicraftServerThread getAssociatedThread(RemotePlayer player)
        ...
        public void broadcastEntityUpdate(Entity e, boolean updateSelf)
        {
            if (e.isRemoved())
            {
                if (Game.debug) Console.WriteLine("SERVER: Tried to broadcast update of removed entity: " + e);
                return;
            }
        ...
        public void broadcastEntityAddition(Entity e, boolean addSelf)
        {
            if (e.isRemoved())
            {
                if (Game.debug) Console.WriteLine("SERVER: Tried to broadcast addition of removed entity: " + e);
                return;
            }
            ...
            int cnt = 0;
            if (Game.debug && e is Player) Console.WriteLine("SERVER: Broadcasting player addition of " + e);
            for (MinicraftServerThread thread: getAssociatedThreads(players))
            {
                thread.sendEntityAddition(e);
                cnt++;
            }
            if (Game.debug && e is Player) Console.WriteLine("SERVER: Broadcasted player addition of " + e + " to " + cnt + " clients");
        }
        ...
        private List<RemotePlayer> getPlayersToRemove(Entity e, boolean removeSelf)
        {
            ...
            if (Game.debug && e is Player)
            {
                Console.WriteLine("SERVER: Sending removal of player " + e + " to " + players.size() + " players (may remove equal player): ");
            ...
            if (Game.debug && e is Player) Console.WriteLine("...now sending player removal to " + players.size() + " players.");
        ...
        public void broadcastEntityRemoval(Entity e, Level level, boolean removeSelf)
        {
            ...
            if (level == null)
            {
                if (Game.debug) Console.WriteLine("SERVER: Cannot remove entity " + e + " from specified level, level given is null; ignoring request to broadcast entity removal.");
                return;
            }
        ...
        public void updateGameVars(MinicraftServerThread[] sendTo)
        {
            //if (Game.debug) Console.WriteLine("SERVER: updating game vars...");
        ...
        {
            Console.WriteLine("Pinging clients (" + threadList.size() + " connected)...");
            for (MinicraftServerThread thread: getThreads())
                thread.doPing();
        }
        ...
        boolean parsePacket(MinicraftServerThread serverThread, InputType inType, String alldata)
        {
            ...
            {
                if (Game.debug) Console.WriteLine(serverThread + " received an error:");
            ...
            {
                case PING:
                    Console.WriteLine("Received ping from " + serverThread);
                    return true;

                case LOGIN:
                    if (Game.debug) Console.WriteLine("SERVER: Received login request");
                    if (Game.debug) Console.WriteLine("SERVER: Login data: " + Arrays.toString(data));
                    ...
                    if (serverThread.getClient().getIpAddress().isLoopbackAddress() && hostPlayer == null)
                    {
                        ...
                        if (Game.debug) Console.WriteLine("SERVER: Host player found");
                    ...
                    if (Game.debug) Console.WriteLine("SERVER: Sending INIT packet");
                    serverThread.sendData(InputType.INIT, sendString.toString());
                    return true;

                case LOAD:
                    if (Game.debug) Console.WriteLine("SERVER: Received level data request");
                    ...
                    for (Entity curEntity : entities)
                    {
                        ...
                        if (curEntity != clientPlayer)
                        {
                            ...
                            if (Game.debug && curEntity is Player)
                                Console.WriteLine("SERVER: Sending player in ENTITIES packet: " + curEntity);
                        }
                ...
                case DIE:
                    if (Game.debug) Console.WriteLine("Received player death");
                ...
                case ENTITY:
                    ...
                    if (entityToSend == null)
                    {
                        ...
                        if (Game.debug) Console.WriteLine("SERVER: Ignoring request to add unknown entity (probably already removed): " + enid);
                        return false;
                    }
                    ...
                    if (Game.debug) Console.WriteLine("SERVER: Sending entity addition via " + serverThread + " because client requested it: " + entityToSend);
                    serverThread.sendEntityAddition(entityToSend);
                    return true;

                case SAVE:
                    if (Game.debug) Console.WriteLine("SERVER: Received player save from " + serverThread.getClient());
                    ...
                    {
                        if (Game.debug) Console.WriteLine("SERVER: Identified SAVE packet client as host");
                ...
                case CHESTOUT:
                    ...
                    { /// inType == InputType.CHESTOUT
                        ...
                        if (!wholeStack && toRemove is StackableItem && ((StackableItem)toRemove).count > 1)
                        {
                ...
                case PUSH:
                    ...
                    else if (!(furniture is Furniture))
                    {
                ...
                case BED:
                    if (Game.debug) Console.WriteLine("Received bed request: " + alldata);
                    boolean getIn = Boolean.parseBoolean(data[0]);
                    if (getIn)
                    {
                        Entity bed = Network.getEntity(Integer.parseInt(data[1]));
                        if (!(bed is Bed) || !Bed.checkCanSleep(clientPlayer))
                        {
                            updateGameVars();
                            return false;
                        }
                ...
                default:
                    Console.WriteLine("SERVER: Used default behavior for input type " + inType);
    ...
    public synchronized void endConnection()
        {
            ...
            if (Game.debug) Console.WriteLine("SERVER: Ending connection with threads: " + threadList);
