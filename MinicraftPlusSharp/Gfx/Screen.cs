...
namespace MinicraftPlusSharp.Gfx
{
    public class Screen
    {
        public static readonly int w = Renderer.WIDTH; // width of the screen
        public static readonly int h = Renderer.HEIGHT; // height of the screen
        public static readonly Point center = new(w / 2, h / 2);

        private static readonly int MAXDARK = 128;
        ...
        private static readonly int BIT_MIRROR_X = 0x01; // written in hexadecimal; binary: 01
        private static readonly int BIT_MIRROR_Y = 0x02; // binary: 10
        ...
        {
            Array.Copy(pixelColors, 0, pixels, 0, Math.Min(pixelColors.Length, pixels.Length));
        }
        ...
        public void Render(int xp, int yp, int tile, int bits, int sheet, int whiteTint, bool fullbright)
        {
            ...
            bool mirrorX = (bits & BIT_MIRROR_X) > 0; // horizontally.
            bool mirrorY = (bits & BIT_MIRROR_Y) > 0; // vertically.
		...
		*/
        private static readonly int[] dither = new int[] {
            0, 8, 2, 10,
            12, 4, 14, 6,
            3, 11, 1, 9,
            15, 7, 13, 5
        ...
        public void Overlay(Screen screen2, int currentLevel, int xa, int ya)
        {
            ...
            for (int y = 0; y < h; y++)
            { // loop through height of screen
                for (int x = 0; x < w; x++)
                { // loop through width of screen
